...
### 1.0.0에서 링크가 표시되지 않는 경우

1.0.0은 Paper 기본 채팅 형식인 `chat.type.text`의 인자 안에 들어 있는 메시지를 처리하지 못합니다. 플러그인이 정상 활성화되어도 URL에 클릭 동작이 추가되지 않을 수 있습니다. 1.0.1은 해당 인자까지 처리합니다.

2026-09-22 Semion 점검에서 실제 실행 버전과 설치 JAR가 모두 1.0.0임을 확인했습니다. 서버에서 가져온 JAR와 1.0.1에 같은 기본 채팅 메시지를 넣어 비교했으며, 이전 JAR에는 `open_url`이 없고 수정 JAR에는 생성되었습니다. KAKC 바이너리 연동을 포함한 테스트 10개도 통과했습니다.

1.0.1을 Semion과 로컬 서버의 `plugins/update/ClickLink.jar`에 준비했습니다. 이번 점검에서는 서버를 재시작하지 않았으므로 아직 실행 버전은 바뀌지 않았습니다. 다음 정상 서버 재시작에 적용됩니다. `/clicklink reload`는 설정만 갱신하며 JAR를 교체하지 않습니다. `/mud update`는 MC Luck Defense만 갱신하므로 ClickLink 업데이트에는 적용되지 않습니다.
